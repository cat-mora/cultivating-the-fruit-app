...
if (!supabaseUrl || !supabaseServiceKey) {
  ...
  console.error(
    "Please set EXPO_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in app/.env",
  );
  process.exit(1);
}
      ...
      try {
        ...
        if (error) {
          ...
          if (directError && directError.message.includes("exec_sql")) {
            console.warn(
              "⚠️  Note: Using Supabase client for migrations has limitations.",
            );
            console.warn(
              "   For full migration support, consider installing Supabase CLI:",
            );
