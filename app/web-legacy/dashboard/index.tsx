...
export default function DashboardWeb() {
  ...
  const completedToday =
    progress?.last_completed_date === new Date().toISOString().split("T")[0];
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      ...
      alert(
        `🎉 Activity Complete! Your streak is now ${streak + 1} days. Keep it going!`,
      );
    } catch (error) {
  ...
  const selectedActivity =
    content.activities.find((a) => a.duration_minutes === selectedTier) ||
  ...
  return (
    ...
    >
      <div
        className="flex flex-col flex-1 max-w-2xl mx-auto w-full px-4 pt-3 pb-4"
      ...
      >
        ...
        <header className="mb-2">
          <div className="flex items-center gap-2 mb-0">
            ...
            <h1 className="text-2xl font-serif text-wine">
              {content.fruit_theme}
            </h1>
          </div>
          <p className="text-charcoal/50 text-xs font-semibold ml-1">
        ...
        <article className="bg-rose-dark p-4 rounded-[20px] shadow-lg mb-3 relative">
          <span className="text-white/40 text-3xl font-serif absolute top-2 left-4">
            "
          </span>
        ...
        >
          {timeTiers.map((tier) => (
            ...
            >
              <span
                className={`text-xs font-bold ${selectedTier === tier ? "text-white" : "text-charcoal/40"}`}
              >
        ...
        {selectedActivity && (
          <section
            className="bg-blush p-4 rounded-[20px] flex-1"
            style={{ overflow: "hidden" }}
          >
            <h2 className="text-xl font-serif text-wine mb-1">
