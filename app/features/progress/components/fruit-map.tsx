    ...
    <View className="gap-6">
      ...
      <View className="bg-parchment border border-cream-dark p-4 rounded-[16px]">
        ...
        <Text className="text-charcoal/60 text-xs leading-5">
          Fruits now fill in gradually as related activities are completed, not
          only when the whole fruit is finished.
        </Text>
      </View>
      ...
      <View className="bg-cream border border-cream-dark p-4 rounded-[16px]">
        ...
        <Text className="text-charcoal/60 text-xs leading-5">
          Every completed activity counts here, including days themed around
          topics like Admiration or Unity.
        </Text>
      </View>
            ...
            >
              ...
              <Text
                className={`text-xs font-semibold text-center ${
                  hasStarted ? "text-charcoal" : "text-charcoal/40"
                }`}
              >
                {label}
              </Text>
              ...
              <View className="w-full h-1.5 bg-white/50 rounded-full mt-3 overflow-hidden">
                <View
                  className={
                    fruit.isCompleted ? "h-full bg-sage" : "h-full bg-wine"
                  }
                  style={{ width: `${fruit.progressPercentage}%` }}
                />
              </View>
              {fruit.isCompleted && <Text className="text-2xl mt-1">✓</Text>}
            </View>
          );
        })}
      </View>
      ...
      {/* Detailed Progress */}
      <View className="bg-parchment border border-cream-dark p-4 rounded-[16px]">
        <Text className="text-wine font-semibold mb-3">
          Completion Timeline
        </Text>
