...
const streams: {
...
}[] = [
  {
    id: "strengthen",
    label: "Strengthen",
    description: "For marriages that are steady and ready to grow deeper.",
    emoji: "🌿",
  },
  {
    id: "repair",
    label: "Repair",
    description:
      "For marriages that feel distant, strained, or in need of restoration.",
    emoji: "🌱",
  },
];
...
const streamColors: Record<
  JourneyStream,
  { selected: string; border: string; text: string }
> = {
  strengthen: {
    selected: "bg-mint border-sage",
    border: "border-sage",
    text: "text-sage",
  },
  repair: {
    selected: "bg-blush border-rose",
    border: "border-rose",
    text: "text-rose-dark",
  },
  family: {
    selected: "bg-cream-dark border-gold",
    border: "border-gold",
    text: "text-gold",
  },
};
...
export default function OnboardingScreen() {
  ...
  const [selectedStream, setSelectedStreamLocal] =
    useState<JourneyStream | null>(null);
  const [selectedTranslation, setSelectedTranslationLocal] =
    useState<BibleTranslation>("NIV");
    ...
    <ScrollView className="flex-1 bg-cream p-6">
      <View className="mt-8 mb-8 items-center">
        ...
        <Text className="text-charcoal/70 text-lg text-center">
          Choose the daily path that best fits your marriage right now.
        </Text>
      </View>
          ...
          return (
            ...
            >
              ...
              >
                ...
                <Text
                  className={`text-xl font-bold ${isSelected ? colors.text : "text-charcoal"}`}
                >
                  {stream.label}
                </Text>
              </View>
      ...
      <View className="mb-12">
        <Text className="text-xl font-serif text-wine mb-4">
        ...
        <View className="flex-row flex-wrap gap-2">
          {translations.map((t) => (
            ...
            >
              <Text
                className={`font-semibold ${selectedTranslation === t ? "text-white" : "text-charcoal"}`}
              >
                {t}
              </Text>
            </Pressable>
