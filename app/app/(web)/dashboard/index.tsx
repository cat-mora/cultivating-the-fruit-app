    ...
    <div className="flex-1 bg-cream min-h-screen overflow-y-auto">
      <div className="max-w-2xl mx-auto px-6 pt-10 pb-8">
        <header className="mb-6">
          <div className="flex items-center justify-between gap-4 mb-3">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-4xl font-serif text-wine">
                {displayContent.fruit_theme}
              </h1>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`w-9 h-9 rounded-full flex items-center justify-center text-lg font-bold ${
                  canGoPrev
                    ? "bg-wine text-white"
                    : "bg-cream-dark text-charcoal/20"
                }`}
              >
                ‹
              </button>
        ...
        <article className="bg-rose-dark p-7 rounded-[28px] shadow-lg mb-5 relative">
          <span className="text-white/40 text-5xl font-serif absolute top-4 left-5">
            "
          </span>
        ...
        <nav
          className="flex items-center justify-center gap-2 mb-5 py-3"
          role="group"
        ...
        >
          {timeTiers.map((tier) => (
            ...
            >
              <span
                className={`text-xs font-bold ${selectedIndex === tier.index ? "text-white" : "text-charcoal/40"}`}
              >
                {tier.label}
              </span>
            </button>
        ...
        {selectedActivity && (
          <section className="bg-blush p-6 rounded-[28px] mb-6">
            <h2 className="text-2xl font-serif text-wine mb-2">
            ...
            >
              <span
                className={`text-lg font-bold ${isPreviewing ? "text-charcoal/40" : "text-white"}`}
              >
                {isPreviewing
                  ? "Preview Only"
                  : completedToday
