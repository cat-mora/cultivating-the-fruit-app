...
export default function SettingsScreen() {
  ...
  const { selectedStream, selectedTranslation, setStream, setTranslation } =
    useUserStore();
  ...
  const handleChangePassword = async () => {
    ...
    if (!isSupabaseEnabled) {
      if (typeof window !== "undefined") {
        window.alert(
          "Password change is only available when using cloud authentication.",
        );
      }
      return;
    }
  ...
  return (
    <ScrollView className="flex-1 bg-cream p-6">
      <View className="mt-14 mb-8">
        ...
        <Text className="text-lg font-bold text-charcoal mb-4">
          Journey Stream
        </Text>
      ...
      >
        <View className="flex-1 justify-center items-center bg-black/50">
          <View className="bg-white rounded-[24px] p-8 mx-6 w-full max-w-md">
            <Text className="text-2xl font-bold text-wine mb-6">
              Change Password
            </Text>

            <View className="mb-4">
              <Text className="text-sm font-semibold text-charcoal mb-2">
                New Password
              </Text>
              <TextInput
                value={newPassword}
